...
\textit{backtracking} \parencite{roughgarden_algorithms}: para cada anime se prueban todas las cantidades posibles
...
actualiza la satisfacción global si la actual es mayor \parencite{gfg_brute_force}.
...
La complejidad espacial es $\mathcal{O}(n)$.


\vspace{1cm}
Los algoritmos greedy construyen la solución paso a paso, tomando en cada iteración el mejor candidato en ese momento, sin considerar decisiones pasadas. Para el problema se implementaron dos heurísticas con diferentes lógicas \parencite{gfg_greedy_algorithms}.

\subsubsection*{Greedy ratio}
Esta heurística evalúa cada par $(anime_i,\, k)$ — es decir, ver exactamente
$k$ capítulos del anime $i$ — y le asigna una \emph{razón de eficiencia}:
...
Todos los pares se ordenan de mayor a menor \textit{ratio}; luego se recorren en ese orden y se selecciona el primero válido para cada anime (sin repetir la elección
de un mismo anime) \parencite{gfg_fractional_knapsack}.

\paragraph{Complejidad}

La creación de pares es $\mathcal{O}\!\left(\sum_{i=1}^n n\_cap_i\right)$.
...
puede construirse a partir de soluciones óptimas de subproblemas más pequeños \parencite{gfg_01_knapsack} \parencite{roughgarden_algorithms}.
...
es \emph{pseudopolinomial}: polinomial en los valores enteros pero exponencial en binario. Para instancias con $M$ y $E$ acotados, este enfoque produce la solución óptima de forma eficiente, a diferencia de la fuerza bruta (ver \cref{fig:complejidad_empirica}).
...
Los gráficos nos muestran que el comportamiento experimental no está tan alejado del marco teórico sobre la complejidad de cada algoritmo. Las \cref{fig:tiempo_vs_n,fig:tiempo_escalamiento} ilustran la drástica diferencia entre los enfoques. En una escala logarítmica, se observa cómo el tiempo de ejecución del algoritmo de fuerza bruta crece de manera exponencial, volviéndose poco eficiente para valores de $n$ superiores a 25. En contraste, los demás algoritmos muestran un crecimiento polinómico, lo que los hace viables para entradas considerablemente más grandes.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{../code/implementation/data/plots/tiempo_algoritmos.png}
    \caption{Comparación del tiempo de ejecución (en nanosegundos, escala \emph{log}) en función del tamaño de la entrada $n$. Se evidencia el crecimiento exponencial de la fuerza bruta frente al crecimiento polinómico de los otros tres algoritmos.}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{../code/implementation/data/plots/calidad_inputs.png}
    \caption{Comparación entre heurísticas, considerando las diferentes variables $n$.}
    \label{fig:calidad_inputs}
\end{figure}

\subsection*{Importancia de las variables en las heurística}

La \cref{fig:calidad_inputs} nos muestra las variaciones de calidad (\% del óptimo) de ambas heurísticas utilizando técnicas de correlación.

\textit{Greedy ratio} no presenta una correlación relevante con N animes ($r=0{,}29$), minutos disponibles ($r=0{,}12$) ni energía disponible ($r=0{,}02$). Sin embargo, sí se observa una correlación moderada y significativa con el total de capítulos ($r=0{,}48$, $p<0{,}05$). En otras palabras, ni el tamaño de la instancia ni la cantidad de recursos disponibles determinan por sí solos qué tan cerca del óptimo queda la solución, aunque la cantidad total de capítulos parece tener cierta influencia.

\textit{Greedy max} no muestra correlación significativa con N animes ($r=0{,}05$) ni con total de capítulos ($r=0{,}05$), a diferencia de lo que podría esperarse dada la lógica de la heurística. En cambio, presenta correlaciones moderadas con minutos disponibles ($r=0{,}35$) y energía disponible ($r=-0{,}36$), aunque ninguna alcanza significancia ($p>0{,}05$ en ambos casos). Esto sugiere que la calidad de \textit{greedy max} está más influenciada por la disponibilidad de recursos que por el tamaño de la instancia.

En definitiva, ninguna variable de entrada resulta fuertemente determinante para ninguna de las dos heurísticas, aunque \textit{greedy ratio} muestra cierta sensibilidad al total de capítulos y \textit{greedy max} a la disponibilidad de recursos.

\subsubsection*{Calidad de Solución}

Si bien la eficiencia es punto importante, la calidad de la solución es el objetivo. Al comparar la satisfacción total obtenida por las heurísticas greedy con la solución óptima usando programación dinámica (ver \cref{fig:boxplot_greedy} y \cref{fig:calidad_greedy}), se observa que la heurística \textit{greedy ratio} de forma consistente encuentra soluciones de mayor calidad que \textit{greedy max}, donde en muchos casos logra dar con el óptimo. Con esto queda en evidencia que la estrategia de \textit{greedy max} tiene una lógica ingenua y con frecuencia queda atrapada en óptimos locales de baja calidad.

\vspace{1cm}
La relación entre el tiempo de ejecución y la calidad de la solución se observan de manera clara en la \cref{fig:tradeoff}. El gráfico nos muestra el costo computacional (tiempo de ejecución) y la satisfacción obtenida (calidad de la solución como porcentaje del óptimo).

\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\textwidth]{../code/implementation/data/plots/tradeoff_calidad_tiempo.png}
...
De la \cref{fig:tradeoff} se puede concluir que:
\begin{itemize}
    \item \textbf{Programación Dinámica (DP)}: Garantiza la solución óptima (100\% de calidad) pero con el mayor tiempo de ejecución. Es la elección correcta cuando la optimalidad es un requisito estricto.
    \item \textbf{Greedy ratio}: Ofrece el mejor equilibrio, con una calidad muy cercana al óptimo (alrededor del 80-100\%) pero con órdenes de magnitud menor que DP respecto del tiempo de ejecución. Si nuestro problema nos permite tolerar una pequeña desviación respecto del óptimo, esta heurística es la correcta.
