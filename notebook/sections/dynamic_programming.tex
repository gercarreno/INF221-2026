\section{Strategies and Base Concepts}


\subsection{Dynamic Programming: General Strategy}
Dynamic Programming (DP) is a systematic way to avoid doing the same work twice.
If a problem feels like magic, it is usually just a well-defined DP state.

\subsubsection{Step 1: Identifying a DP Problem}
Do not waste time on DP if a simple Greedy approach works.
A problem requires DP if it triggers these conditions:
\begin{itemize}
...
\begin{itemize}
    ...
    \item \textbf{State Transition:} The mathematical formula that tells you how to calculate the current state from previous states.
    It usually looks like a \texttt{max()} or \texttt{min()} function choosing between two paths (e.g., \textit{``Do I take this item, or do I leave it?''}).
...
\section{Combinatorics and Coin Change}

\subsection{Dice Combinations [CSES]}
...
\subsection{Minimizing Coins [CSES]}
...
\subsection{Coin Combinations I \& II [CSES]}
\textbf{Concept:} 
\textit{Coin Combinations I} cares about permutations (order matters).
The outer loop is the target weight.
\textit{Coin Combinations II} cares about combinations (order DOES NOT matter).
The outer loop is the coins array.
...
\subsection{Removing Digits [CSES]}
...
\subsection{Problem B: Restaurant Orders [Kattis]}
...
void processOrders(int num_items, const vector<int>& costs, int num_orders, const vector<int>& queries) {
    ...
    // dp array tracks NUMBER of ways to make a cost.
    // We cap it at 2 to prevent overflow.
...
\textbf{Concept:} Maximize the value in a knapsack of limited capacity.
You must take an item entirely or leave it (no fractions).
\\
...
\subsection{Money Sums \& Two Sets II [CSES]}
\textbf{Concept:} Subset Sum variants. \textit{Money Sums} finds all possible sums.
\textit{Two Sets II} finds the number of ways to partition an array into two equal subsets.
...
\subsection{Problem A: Knapsack [Kattis]}
...
\subsection{Problem C: Presidential Elections [Kattis]}
\textbf{Concept:} Minimizing weight for a target value.
Weight = "Voters needed to swing state", Value = "Electoral Delegates".
...
int32_t main() {
    ...
    for (int i = 0; i < num_states; ++i) {
        ...
        if (const_c >= votes_needed_to_win) {
            guaranteed_delegates += delegates;
            // Already won
        } else {
    ...
    vector<int> dp(total_delegates + 1, 1e15);
    // Use 1e15 for infinity
    dp[0] = 0;
...
\section{Jumps and Sequences 1D}

\subsection{Maximum Weight Independent Set (MWIS) on a Path}
\textbf{Concept:} Also known as the "House Robber" problem.
Find the maximum subset sum of an array such that no two selected elements are adjacent.
\\
...
\textbf{Concept:} Find the minimum cost to reach the last stone.
You can jump 1 or 2 steps. Cost is the absolute difference in height.
...
\subsection{Problem D: Spiderman's Workout [Kattis]}
\textbf{Concept:} A 2D DP pathfinding problem.
We must track the minimum *maximum height* reached, and strictly stay above height 0.
...
void solveSpiderman(int num_distances, const vector<int>& distances) {
    int max_possible_height = 1000;
    // Sum of all distances is <= 1000
    ...
    for (int i = 0; i < num_distances; ++i) {
        ...
        for (int h = 0; h <= max_possible_height; ++h) {
            ...
            if (h - dist >= 0) {
                int peak_if_down = dp[i][h];
                // Going down doesn't increase max height
...
\section{Grid, Matrices and Shapes}
\subsection{Matrix Chain Multiplication (Interval DP)}
\textbf{Concept:} Find the most efficient way to multiply a given sequence of matrices. 
\textbf{State:} Unlike standard DP that iterates linearly, Interval DP iterates by the \textit{length of the interval} (\texttt{L}). \texttt{dp[i][j]} represents the minimum cost to multiply matrices from index \texttt{i} to \texttt{j}.
...
// dims array represents matrix dimensions. 
...
int matrixChainMultiplication(const vector<int>& dims) {
    ...
    // dp[i][j] = min scalar multiplications to compute matrix A[i]...A[j]
    vector<vector<int>> dp(num_matrices + 1, vector<int>(num_matrices + 1, 0));

    // Time Complexity: O(N^3)
    // Base case: L = 1 (single matrix) costs 0, already initialized.
    
    // L is the length of the interval (chain of matrices) we are evaluating
    for (int L = 2; L <= num_matrices; ++L) {
        for (int i = 1; i <= num_matrices - L + 1; ++i) {
            ...
            for (int k = i; k < j; ++k) {
                // Cost = Cost of Left Chain + Cost of Right Chain + Cost of multiplying them together
                int current_cost = dp[i][k] + dp[k + 1][j] + (dims[i - 1] * dims[k] * dims[j]);
                dp[i][j] = min(dp[i][j], current_cost);
            }
        }
    }
    
    // Return the minimum cost to multiply the entire chain from 1 to N
    return dp[1][num_matrices];
}
\end{lstlisting}

\subsection{Grid Paths [CSES]}
...
\subsection{Rectangle Cutting [CSES]}
\textbf{Concept:} Split a WxH rectangle into squares.
...
\textbf{Concept:} Move from top to bottom.
Optimized by going bottom-up, reducing space to 1D.
...
\section{Subsequences and Strings}

\subsection{Increasing Subsequence [CSES]}
...
\subsection{Problem E: Increasing Subsequence [Kattis]}
\textbf{Concept:} The problem specifically asks for the \textit{lexicographically smallest} sequence in the case of a tie.
Because $N \le 200$, we can use an $O(N^2)$ approach and trace backward carefully.
...
void printLexicographicallySmallestLIS(int n, const vector<int>& arr) {
    // dp[i] = length of LIS starting at index i. Space: O(N)
    vector<int> dp(n, 1);
...
bool canBreakWord(const string& target, const vector<string>& word_dict) {
    ...
    // Space: O(N) - dp[i] is true if the prefix of length 'i' can be segmented
...
\section{Graphs and Short Path [DP]}

\subsection{Bellman-Ford Algorithm}
\textbf{Concept:} A graph DP algorithm to find the shortest path from a single source to all vertices.
Unlike Dijkstra, it correctly handles negative edge weights and detects negative weight cycles.
\\
...
\subsection{Problem F \& G: Bellman-Ford with Negative Cycles [Kattis]}
...
void solveBellmanFordNegativeCycles(int num_vertices, int num_edges, int source, int num_queries) {
    ...
    // Space: O(V)
    vector<int> distances(num_vertices, 1e15);
    // 1e15 represents Infinity
    distances[source] = 0;
    
    // Step 1: Relax edges V-1 times. Time: O(V * E)
    for (int i = 0; i < num_vertices - 1; ++i) {
    ...
    // Step 2: Propagate Negative Infinity. Run V-1 more times. 
    // If a node can STILL be relaxed, it is part of, or reachable from, a negative weight cycle.
    // Time: O(V * E)
    for (int i = 0; i < num_vertices - 1; ++i) {
        for (const Edge& e : edges) {
            if (distances[e.u] != 1e15 && distances[e.u] + e.weight < distances[e.v]) {
...
\section{Mathematics and State Machines}

\subsection{Array Description [CSES]}
...
\section{Advanced Topics (Bitmask and Intervals)}

\subsection{Projects (Weighted Interval Scheduling) [CSES]}
\textbf{Concept:} Sort jobs by end time.
Use binary search (\texttt{lower\_bound}) to find the latest non-overlapping project.
...
\subsection{Elevator Rides (Bitmask DP) [CSES]}
\textbf{Concept:} Minimize elevator trips for $N$ people ($N \le 20$).
We use a bitmask where the $i$-th bit represents whether person $i$ is in the elevator.
\\
...
int getMinElevatorRides(int n, int max_weight, const vector<int>& weights) {
    int max_mask = (1 << n);
    // Space: O(2^N)
    vector<State> dp(max_mask, {25, 0});
    // 25 is safe infinity for N=20
